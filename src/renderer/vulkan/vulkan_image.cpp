...
namespace Raytracing
{
    ...
    {
        imageView = VulkanUtils::CreateImageView(device->GetDevice(), image, format, aspectFlags);
    }
    ...
    VkDeviceMemory VulkanImageBuilder::AllocateImageMemory(VulkanDevice* device, VkImage image, VkMemoryPropertyFlags properties)
    {
        VkMemoryRequirements memRequirements;
        vkGetImageMemoryRequirements(device->GetDevice(), image, &memRequirements);
    ...
    VkImage VulkanImageBuilder::CreateImage(VulkanDevice* device, VkImageUsageFlags usage)
    {
        VkImageCreateInfo imageInfo{};
        imageInfo.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
        imageInfo.imageType = VK_IMAGE_TYPE_2D;
    ...
    VkImageView VulkanImageBuilder::CreateImageView(VulkanDevice* device, VkImageAspectFlags aspectFlags)
    {
    ...
    VkSampler VulkanImageBuilder::CreateSampler(VulkanDevice* device)
    {
        VkPhysicalDeviceProperties properties{};
        vkGetPhysicalDeviceProperties(device->GetPhysicalDevice(), &properties);
    ...
    void VulkanImageBuilder::TransitionImageLayout(VulkanDevice* device, VkImageLayout oldLayout, VkImageLayout newLayout)
    {
        VkCommandBuffer commandBuffer = VulkanUtils::BeginSingleTimeCommands(device->GetDevice(), device->GetCommandPool());
    ...
    void VulkanImageBuilder::CopyBufferToImage(VulkanDevice* device, VkBuffer buffer)
    {
        VkCommandBuffer commandBuffer = VulkanUtils::BeginSingleTimeCommands(device->GetDevice(), device->GetCommandPool());
